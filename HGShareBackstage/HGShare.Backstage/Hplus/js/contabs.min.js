...
var tabTools =
{
    tablogingdialog: null,
    /*tab loging...*/
    TabLoging: function () {
        tablogingdialog = layer.load(0, { shade: [0.1, '#393D49'],time:15000});
            $(".J_mainContent iframe:visible").load(function () {
                layer.close(tablogingdialog);
            })
    },
    /*tab loging close ..*/
    TabLogingClose: function () {
        if (tablogingdialog) {
            layer.close(tablogingdialog);
        }
    },
    /*返回当前tab初始页*/
    GoToCurrentTabHome: function () {
    ...
    /*执行当前tab方法*/
    CurrentTabExec: function (funname) {
        var iframe = this.GetCurrentTab();
        if (iframe && iframe[funname])
            iframe[funname]();
    },
    /*得到当前tab iframe*/
    GetCurrentTab: function () {
        var dataindex = $(".J_menuTab.active").attr("data-index");
        if (dataindex)
            return window['iframe' + dataindex];
        else
            return window;
    },
    TabPageReload: function () {
        tabTools.CurrentTabExec("TabPageReload");
        tabTools.TabLoging();
    }
}
...
$(function () {
    ...
    function n() {
        ...
        if ($(".J_menuTab").each(function() {
            ...
            var s = '<a href="javascript:;" class="active J_menuTab" data-index="' + a + '" data-id="' + t + '">' + i + ' <i class="fa fa-times-circle"></i></a>';
            ...
            var aiframe = $(".J_iframe[name='iframe" + a + "']");
            if (aiframe) {
                tabTools.GoToCurrentTabHome();
            }
        }
